...
(function($){
  ...
  $(function(){
    ...
    $('#popup_ldfds').modal({
      dismissible: false, // Modal can be dismissed by clicking outside of the modal                opacity: .5, // Opacity of modal background
      opacity: .3, // Opacity of modal background
      endingTop: '5%', // Ending top style attribute
      complete: function(){
        Cookies.set('popup_ldfds', '1', { expires: 2});
      } // Callback for Modal close
    });
    $('#popup_ldfds a.gotoform').click(function(){
      Cookies.set('popup_ldfds', '1', { expires: 2});
    });
    if(Cookies.get('popup_ldfds') != '1'){
      //$('#popup_ldfds').modal('open');
    }
    ...
    $('.section-opinion .ld-card').matchHeight();

    // Non CSS Grid enabled browsers:
    if (!Modernizr.cssgrid){
      // Match height of cards.
      $('.frontpage-section .ld-card').matchHeight();
      $('.ld-grid-area-col').matchHeight();
    }
