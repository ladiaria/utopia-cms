...
class AddRatingView(object):
    ...
    def rating_changed_response(self, request, context):
        if request.is_ajax():
          return HttpResponse(simplejson.dumps({"likes":context['field'].likes, "dislikes":context['field'].dislikes, "message":'Vote changed.'}), mimetype='application/json')
        response = HttpResponse('Vote changed.')
        return response

    def rating_added_response(self, request, context):
      if request.is_ajax():
        return HttpResponse(simplejson.dumps({"likes":context['field'].likes, "dislikes":context['field'].dislikes, "message":'Vote recorded.'}), mimetype='application/json')
      response = HttpResponse('Vote recorded.')
      return response

    def authentication_required_response(self, request, context):
      if request.is_ajax():
        return HttpResponse(simplejson.dumps({"message":'login required'}), mimetype='application/json')
