...
class AnonymousRequest(object):
    def process_request(self, request):
        ...
        if getattr(settings, 'HOME_CACHE_DEBUG', False) and request.path == '/':
            print("DEBUG: cache.process_request: anon=%s, COOKIE header=%s, session_key=%s" % (
                request.user.is_anonymous(), request.META.get('HTTP_COOKIE'),
...
class AnonymousResponse(object):
    def process_response(self, request, response):
        ...
        if getattr(settings, 'HOME_CACHE_DEBUG', False) and request.path == '/':
            print("DEBUG: cache.process_response: anon=%s, COOKIE header=%s, session_key=%s" % (
                request.user.is_anonymous(), request.META.get('HTTP_COOKIE'),
                request.COOKIES.get(settings.SESSION_COOKIE_NAME)))
