...
from .utils import get_active_tasks, get_scheduled_tasks
...
def update_category_home():
    ...
    task_name = update_category_home_task.name
    if not (get_active_tasks(task_name) or get_scheduled_tasks(task_name)):
        try:
