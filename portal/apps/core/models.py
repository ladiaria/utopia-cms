...
# NOTE: Do not indent model docstrings, they break django admin autogenerated documentation.
...
class PortableDocumentFormatBaseModel(Model):
    """
Abstract base model to be used as the parent class for Edition and Supplement models.
**Non-default indexed fields:** date_published.
    """
    pdf = FileField(
        'archivo PDF',
        max_length=150,
...
class Edition(PortableDocumentFormatBaseModel):
    """
A publication's edition. **Non-default indexed fields:** date_published (defined in parent model).
    """
...
class Article(ArticleBase):
    """
Article model, the main model to use for journalistic publishing.
**Non-default indexed fields:**
tuple(type, date_published, is_published), type, url_path, is_published, to_be_published, date_published,
date_created, last_modified, views, allow_comments, allow_related.
All defined in parent abstract model ArticleBase.
    """
...
class ArticleViewedBy(Model):
    """
Model to track which articles have been viewed by which users.
**Non-default indexed fields:** viewed_at.
    """
...
class ArticleViews(Model):
    """
Model to track the number of views for an article by day.
**Non-default indexed fields:** day.
    """
    article = ForeignKey(Article, on_delete=CASCADE)
...
class ArticleUrlHistory(Model):
    """
Model to track the history of URLs for an article and support the redirect functionality for old URLs.
**Non-default indexed fields:** absolute_url.
    """
    article = ForeignKey(Article, on_delete=CASCADE)
...
class PerplexityAPISettings(SingletonModel):
    ...
    result_instructions = TextField(
        default=(
            ...
            'y adecuado para Google Discover, siguiendo el estilo de la diaria.\n'
