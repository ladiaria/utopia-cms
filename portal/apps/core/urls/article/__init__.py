...
from core.views.article import article_detail_walled, article_detail_free, send_by_email, article_detail_ipfs, coral_comment_count


urlpatterns = [
    path("enviar/", send_by_email, name="send_by_email"),
    re_path(r"^ipfs/(?P<article_id>\d+)/$", article_detail_ipfs, name="ipfs-article-detail"),
    re_path(r"^(?P<article_id>\d+)/comment-count/$", coral_comment_count, name="coral-comment-count"),
    re_path(
