...
from utopia_cms_ladiaria.models import RadioNewsletter
...
def get_profile_newsletters_ordered():
    ...
    newsletters = [nl_obj for nl_obj in nl_custom_ordered if nl_obj] + nl_alpha

    # Add RadioNewsletter if it is not already in the list
    # This allows RadioNewsletter to appear in the user dashboard
    radio_newsletter = RadioNewsletter.objects.order_by("-day").first()
    if radio_newsletter and radio_newsletter not in newsletters:
        newsletters.append(radio_newsletter)
