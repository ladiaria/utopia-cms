var InlineOrdering = {
    ...
    init: function () {
        ...
        // Rompemos el tbody para hacer multiples tbody, es lo más semántico que pude hacer.
...
//cambia el link de crear un inline por un popup
var PopUpAdd = {
    init: function () {
        $('tr.add-row a').each(function(){
            var add_link = $(this);
            var section_id = add_link.closest('div[id^=articlerel_set-].inline-group').attr('data-section_id');
            var pub_id = $('#id_publication').val();
            add_link.off("click");
            /**///add_link.attr('href','/admin/core/article/add?type=NE&edition='+$('#id_articlerel_set-0-edition').val() + '&sections=' + section_id + '&publication=' + pub_id);
...
//refresh de la edición cuando volves del popup
...
$(function () {
    $(document).load(InlineOrdering.init());
    $(document).load(SectionIds.init());
    $(document).load(PopUpAdd.init());
});
